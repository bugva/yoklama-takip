...
/** GPA verilerini ayrıca yükler/kaydeder */
const GPA_KEY = 'gpa-v5'
export function loadGpaData(): any {
  try {
    const raw = localStorage.getItem(GPA_KEY)
    return raw ? JSON.parse(raw) : null
  } catch { return null }
}
export function saveGpaData(data: any): void {
  if (data) localStorage.setItem(GPA_KEY, JSON.stringify(data))
  else localStorage.removeItem(GPA_KEY)
}
...
export function exportJson(data: AppData): string {
  const gpa = loadGpaData()
  return JSON.stringify({
    ...data,
    version: CURRENT_VERSION,
    gpaData: gpa // GPA verisini yedek dosyasına ekle
  }, null, 2)
}
...
export function importJson(text: string): AppData | null {
  try {
    ...
    const normalized = normalizeAppData(parsed)
    if (normalized && parsed.gpaData) {
      saveGpaData(parsed.gpaData) // GPA verisini geri yükle
    }
    return normalized
  } catch {
    return null
